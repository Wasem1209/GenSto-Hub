...
export default function ServicesPage() {
  const [techSolutions, setTechSolutions] = useState([]);
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        const response = await fetch(`${REST_API}/services`);
        const result = await response.json();
        if (result.success) {
          setTechSolutions(result.data);
        }
      } catch (error) {
  ...
  const handleOpenModal = (solution) => setSelectedSolution(solution);
  ...
  return (
    <section className="bg-gray-50 mt-8 py-16 px-6 relative min-h-[60vh]">
            ...
            {techSolutions.map((solution, index) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-gray-500 text-sm mt-3 leading-relaxed">{solution.description}</p>
      ...
      {selectedSolution && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
          <div className="bg-white rounded-[2.5rem] w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl relative animate-in fade-in zoom-in-95 duration-200">
            <button onClick={handleCloseModal} className="absolute top-8 right-8 p-2 hover:bg-gray-100 rounded-full transition">
            ...
            {!showSuccess ? (
              <div className="p-10 md:p-14">
                ...
                <div className="grid grid-cols-3 gap-4 mb-10">
                  <a href="#" className="flex flex-col items-center p-4 rounded-2xl bg-green-50 text-green-600 hover:bg-green-100 transition border border-green-100">
                    <MessageCircle className="w-7 h-7 mb-2" />
                    <span className="text-[10px] font-black uppercase">WhatsApp</span>
                  </a>
                  <a href="#" className="flex flex-col items-center p-4 rounded-2xl bg-blue-50 text-blue-600 hover:bg-blue-100 transition border border-blue-100">
                    <Mail className="w-7 h-7 mb-2" />
                    <span className="text-[10px] font-black uppercase">Email</span>
                  </a>
                  <a href="#" className="flex flex-col items-center p-4 rounded-2xl bg-gray-50 text-gray-600 hover:bg-gray-100 transition border border-gray-200">
              ...
              <div className="p-16 text-center flex flex-col items-center">
                ...
                <p className="text-gray-500 mt-4 leading-relaxed max-w-sm">
                  The <span className="font-bold text-blue-600">INANST</span> team has been notified regarding <strong>{selectedSolution.title}</strong>.
                </p>
                <button onClick={handleCloseModal} className="mt-10 bg-gray-900 text-white px-12 py-5 rounded-2xl font-black hover:bg-black transition uppercase tracking-widest text-xs">
                  Back to Solutions
                </button>
              </div>
            )}
