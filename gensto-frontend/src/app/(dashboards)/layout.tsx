...
// Removed useLayoutEffect from the imports below
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!loading) {
      ...
      const rolePath = `/${user.role}`;
      if (pathname === '/dashboard' || (pathname.startsWith('/dashboard') && !pathname.startsWith(rolePath))) {
        router.replace(rolePath);
      }
    }
  ...
  const handleResend = async () => {
    if (!user?.email) {
        console.error("Session data missing during resend attempt");
        return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(errorData.msg || "Failed to send verification email");
      }
    } catch (err) {
        ...
        <div className="fixed inset-0 z-[100] bg-black/40 backdrop-blur-[4px] flex items-center justify-center p-6">
          <div className="max-w-md w-full bg-white rounded-[3.5rem] shadow-2xl p-10 border border-gray-100 text-center animate-in fade-in zoom-in duration-500">
            ...
            <div className="space-y-4">
              ...
              >
                {resending ? <Loader2 className="animate-spin w-5 h-5" /> : <>Send me a code <Send className="w-4 h-4 rotate-45" /></>}
              </button>
