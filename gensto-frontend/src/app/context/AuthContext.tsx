  ...
  const checkAuth = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Handle nested user object if your backend returns { success: true, user: {...} }
        ...
        // If user is unverified, don't kick them out on profile error (allows them to stay on OTP page)
        if (parsedUser && parsedUser.isVerified === false) {
           setLoading(false);
           return;
        }

        // Logout on specific auth failures
        if (res.status === 401 || res.status === 403 || res.status === 404) {
           logout();
        }
      }
    } catch (err) {
      console.error("Auth verification failed (Network/Server):", err);
    } finally {
      setLoading(false);
    }
  }, [logout]);
